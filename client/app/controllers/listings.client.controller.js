angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
      ...
      var listing = {
        name: $scope.name, code: $scope.code, address: $scope.address
      };

      var lisID = $stateParams.listingId;

      Listings.update(lisID, listing).then(function(response){
        $state.go('listings.list', { successMessage: 'The listing was updated successfully.'});
      }, function(error){
        $scope.error = 'Could not update: ' + error;
      });
      /*
    ...
    $scope.remove = function() {
       ...
       */
       $scope.error = null;
       var del = $stateParams.listingId;
       Listings.delete(del).then(function(response){
        $state.go('listings.list', {successMessage: 'The Listing was removed successfully.'});
       }, function(error){
        $scope.error = 'Could not remove: ' + error;
       });
    };
